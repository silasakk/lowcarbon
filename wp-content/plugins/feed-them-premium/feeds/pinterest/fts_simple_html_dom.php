 ...
 * @subpackage fts_simple_html_dom
 */
...
function fts_file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new fts_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $defaultBRText);
...
function fts_str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT)
{
    $dom = new fts_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $defaultBRText);
    if (empty($str))
    {
...
// dump html dom tree
function fts_dump_html_tree($node, $show_attr=true, $deep=0)
{
    $node->dump($node);
}
 ...
 * @package PlaceLocalInclude
 */
class fts_simple_html_dom_node {
...
class fts_simple_html_dom {
    ...
    function load_file() {
        ...
        // Per the fts_simple_html_dom repositiry this is a planned upgrade to the codebase.
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT) {
        ...
        $this->root = new fts_simple_html_dom_node($this);
        $this->root->tag = 'root';
    ...
    protected function parse() {
        ...
        // text
        $node = new fts_simple_html_dom_node($this);
        ++$this->cursor;
    ...
    protected function read_tag() {
        ...
        if ($this->char==='/') {
            $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
            // This represetns the change in the fts_simple_html_dom trunk from revision 180 to 181.
